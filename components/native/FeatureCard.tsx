import DynamicIcon, { IconProps } from "ZC/ui/icon";

type FeatureCardProps = {
  key: string;
  title: string;
  body: string;
  iconProps: Pick<IconProps, "name" | "size" | "className">;
};

export default function FeatureCard({ key, title, body, iconProps }: FeatureCardProps) {
  return (
    <div className="flex min-h-40 min-w-80 flex-col gap-4 rounded-lg border border-yellow-500 p-6" key={key}>
      <DynamicIcon {...iconProps} />
